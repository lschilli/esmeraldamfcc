...
int mx_scorelist_getscore(mx_real_t *score, mx_scorelist_t *scorelist, int id)
	{
	int i;
	...
	/* ... search for matchin entry ... */
	for (i = 0; i < scorelist->n_scores; i++) {
		if (scorelist->score[i].id == id) {
			*score = scorelist->score[i].score;
			return(id);
			}
		}
...
int mx_scoreset_fread(mx_scoreset_t *scoreset, FILE *fp)
	...
	/* if scoreset given is empty ... */
	if (scoreset->n_lists == 0) {
		/* ... create a single (default) scorelist */
		scorelist = mx_scorelist_create_by_type(scoreset->type);
        	mx_scoreset_addlist(scoreset, scorelist);
		}
